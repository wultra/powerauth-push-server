...
public interface InboxRepository extends PagingAndSortingRepository<InboxMessageEntity, String>, CrudRepository<InboxMessageEntity, String> {
     ...
     */
    @Query("SELECT DISTINCT o FROM InboxMessageEntity o INNER JOIN o.applications a WHERE o.userId = :userId AND a.appId in :applicationIds AND o.isRead = :read ORDER BY o.timestampCreated DESC")
