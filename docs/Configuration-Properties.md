...
| `powerauth.push.service.clients.cache.expireAfterWrite`           | `24h`                   | APNS, FCM and HMS client configuration is cached. It is evicted if updated via administration on a single node. This is a fallback for the clustered environment. |
